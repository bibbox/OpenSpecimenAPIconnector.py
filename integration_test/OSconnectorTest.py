...
class integrationTest:
    ...
    def IntegrationTest(self):
        ...
        """
        log.append("-Delete an Institute- \n")
        response = institutes.delete_institute(inid = ID)
        assert bool(response), "error deleting an institute"
        log.append(str(response) + " \n")

        log.append("-Get an Institute- \n")
        response = institutes.get_institute(inid = ID)
        assert bool(response), "error getting an institute"
        log.append(str(response) + " \n")

        return log

    def checkCollectionProtocol(self):

        cp = os_core.collection_protocol(base_url = self.base_url, auth = self.auth)
        cp_util = os_util.collection_protocol_util(base_url = self.base_url, auth = self.auth)
        
        log = ["+++Colelction Protocol+++ \n", "-Create Colelction Protocol-"]
        response = cp_util.create_cp(short_title = "IntTestCP", title = "IntegrationTest", pi_mail = "email@address.com", time_start = "2021-02-23",
                time_end = "2021-02-24", sites
"""
